...
#pragma warning disable IDE0011, S2681
...
public partial class TripsPanel
{
    ...
    private void ApplyTemplate() { var template = Templates.SingleOrDefault(candidate => candidate.Key == newTrip.TemplateKey); if (template is null) return; newTrip.MinimumTemperatureCelsius = template.DefaultMinimumTemperatureCelsius; newTrip.MaximumTemperatureCelsius = template.DefaultMaximumTemperatureCelsius; newTrip.LuggageAllowanceGrams = template.DefaultLuggageAllowanceGrams; newTrip.CabinOnly = template.CabinOnly; }
    ...
    private void SetTraveller(Guid id, object? value) { if (value is bool selected && selected) tripProfileIds.Add(id); else tripProfileIds.Remove(id); }
    ...
    private async Task SaveTraveller() { if (editingTraveller is not null && !string.IsNullOrWhiteSpace(editingTravellerName)) await TravellerUpdated.InvokeAsync(editingTraveller with { Name = editingTravellerName.Trim(), PackingNotes = editingTravellerPackingNotes, MedicalNotes = editingTravellerMedicalNotes }); }
    private void SetUsage(Guid id, object? value) { if (value is bool used && used) usedItemIds.Add(id); else usedItemIds.Remove(id); }
